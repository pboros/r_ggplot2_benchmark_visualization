...
# sysbench_throughput
sysbench_tps<-subset(sysbench_oltp,metric=="sysbench_tps")
...
tps_graph<-tps_graph+aes(x=factor(threads),y=value,geom=storage,colour=storage)
tps_graph<-tps_graph+facet_grid(ro_rw ~ storage)
...
tps_graph<-tps_graph+xlab("Threads")
tps_graph<-tps_graph+ylab("Transactions per second")
tps_graph<-tps_graph+ggtitle("Sysbench OLTP throughput")
tps_graph<-tps_graph+theme(legend.key=element_rect(fill="white"))
...
tps_graph<-tps_graph+theme(axis.text.x = element_text(size = rel(0.8), angle = 45)) 
tps_graph

ggsave(plot = tps_graph, "r_ggplot2_benchmark_visualization/ex16.png", dpi=200, scale=1, height=6, width=6, type = "cairo-png")
