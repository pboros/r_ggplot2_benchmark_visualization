...
tps_graph<-tps_graph+aes(x=time,y=value,geom=storage,color=storage)
tps_graph<-tps_graph+geom_jitter(alpha=0.3)
tps_graph<-tps_graph+expand_limits(y=0)
tps_graph

ggsave(plot = tps_graph, "r_ggplot2_benchmark_visualization/ex5.png", dpi=200, scale=1, height=6, width=6, type = "cairo-png")
